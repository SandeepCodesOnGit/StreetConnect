  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      ...
      ...(role === "vendor" && {
        ...
        category: formData.category.trim().toLowerCase() || "other",
      }),
    };
      ...
      <div className="relative z-10 w-full max-w-xl bg-white/10 backdrop-blur-xl rounded-[2.5rem] shadow-2xl border border-white/30 flex flex-col">
        <div className="p-8 md:p-12">
          ...
          <div className="flex flex-col items-center mb-8 text-white text-center">
            ...
            <div className="flex gap-2 mt-4">
              <div
                className={`h-1.5 w-8 rounded-full transition-all duration-300 ${step === 1 ? "bg-yellow-400" : "bg-white/20"}`}
              ></div>
              <div
                className={`h-1.5 w-8 rounded-full transition-all duration-300 ${step === 2 ? "bg-yellow-400" : "bg-white/20"}`}
              ></div>
            </div>
          </div>
            ...
            >
              ...
              <div className="flex gap-4 pt-4 relative z-0">
                <button
                  ...
                  className="p-4 bg-white/10 rounded-xl text-white hover:bg-white/20 transition-all active:scale-95"
                >
        ...
        <div className="bg-black/20 p-6 text-center border-t border-white/10 mt-auto rounded-b-[2.5rem] relative z-0">
          <p className="text-white/80 text-sm font-medium">
            ...
            <Link
              to="/signin"
              className="ml-2 text-yellow-300 font-bold hover:text-white underline underline-offset-4"
            >
              Sign In
            </Link>
          </p>
...
const CustomDropdown = ({
  ...
  return (
    <div className="relative group z-50">
      ...
      {/* Changed from <span> to <input> to allow custom typing */}
      <div className="relative">
        <input
          ...
          onChange={onChange} // This allows custom typing
          placeholder={placeholder}
          autoComplete="off"
          className={`w-full bg-white/10 border ${isOpen ? "border-yellow-300 ring-2 ring-yellow-300/50" : "border-white/20"} rounded-xl py-4 pl-11 pr-10 text-white placeholder-white/50 focus:outline-none transition-all font-medium capitalize`}
        />
        <div
          className="absolute inset-y-0 right-0 pr-4 flex items-center cursor-pointer text-white/50"
      ...
      {isOpen && (
        <>
          ...
          ></div>
          <div className="absolute z-50 w-full mt-2 bg-black/80 backdrop-blur-3xl border border-white/20 rounded-xl shadow-2xl overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
              ...
              {options
                ...
                .map((opt, idx) => (
                  <div
                    ...
                    className={`px-5 py-3.5 cursor-pointer transition-colors text-white font-medium hover:bg-white/20 capitalize ${value.toLowerCase() === opt.toLowerCase() ? "bg-yellow-400/30 text-yellow-300 border-l-2 border-yellow-400" : ""}`}
                  >
                    {opt}
                  </div>
                ))}

              {/* Show "Create New" option if no match found */}
              {value &&
                !options.some(
                  (opt) => opt.toLowerCase() === value.toLowerCase(),
                ) && (
                  <div className="px-5 py-3.5 text-yellow-300 italic text-sm border-t border-white/10">
