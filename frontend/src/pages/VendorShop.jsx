...
const VendorShop = () => {
  ...
  useEffect(() => {
    ...
    const handleMenuUpdate = (data) => {
      setLiveVendor((prev) => (prev ? { ...prev, menu: data.menu } : prev));
    };

    const handleStatusUpdate = (data) => {
      setLiveVendor((prev) => (prev ? { ...prev, isLive: data.isLive } : prev));
    };
  ...
  return (
    <div className="pt-24 min-h-screen bg-gray-50 pb-40">
      <div className="max-w-5xl mx-auto px-4 lg:px-0">
        ...
        <div className="bg-white rounded-4xl p-6 lg:p-8 shadow-sm border border-gray-100 flex flex-col md:flex-row gap-6 md:gap-8 items-center md:items-start mb-12">
          <div className="w-32 h-32 lg:w-40 lg:h-40 shrink-0 bg-gray-100 rounded-3xl overflow-hidden shadow-inner relative border-4 border-white outline outline-1 outline-gray-100">
            <span
              className={`absolute top-3 left-3 px-3 py-1 rounded-full text-[10px] font-black text-white shadow-md backdrop-blur-md ${liveVendor.isLive ? "bg-green-500/90 animate-pulse" : "bg-gray-800/90"}`}
            >
          ...
          {liveVendor.menu && liveVendor.menu.length > 0 ? (
            liveVendor.menu.map((item, index) => {
              ...
              return (
                <div
                  key={index}
                  className="bg-white rounded-[2rem] overflow-hidden shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 border border-gray-100 flex flex-col group relative"
                >
                  ...
                  <div className="p-6 flex flex-col flex-1">
                    <div className="mb-4">
                      <h4
                        className={`font-black text-2xl mb-2 leading-tight ${item.isAvailable === false ? "text-gray-400" : "text-gray-900"}`}
                      >
                        {item.name}
                      </h4>
                      <p className="text-gray-500 text-sm line-clamp-2 leading-relaxed font-medium">
                        {item.description ||
                          "A delicious street food specialty, made fresh to order."}
                      </p>
                    </div>
                    ...
                    <div className="flex items-center justify-between mt-auto pt-4 border-t border-gray-50">
                      <p
                        className={`font-black text-2xl ${item.isAvailable === false ? "text-gray-400" : "text-orange-500"}`}
                      >
                        ₹{item.price}
                      </p>
                      ...
                      {qty === 0 ? (
                        <button
                          ...
                          disabled={
                            !liveVendor.isLive || item.isAvailable === false
                          }
                        ...
                        <div className="flex items-center gap-3 bg-orange-500 text-white px-2 py-1.5 rounded-xl font-bold shadow-lg shadow-orange-500/30">
                          <button
                            ...
                            className="w-8 h-8 rounded-lg hover:bg-white/20 flex items-center justify-center transition active:scale-90"
                          >
            ...
            <div className="col-span-full text-center py-20 bg-white rounded-3xl border border-gray-100 shadow-sm">
              ...
              <p className="text-gray-400">
                This vendor is still setting up their shop.
              </p>
            </div>
          )}
        </div>
      ...
      {totalItems > 0 && (
        <div className="fixed bottom-6 left-0 right-0 max-w-4xl mx-auto px-4 z-40 animate-fade-in-up">
          <div
            ...
            className="bg-gray-900 text-white p-4 lg:p-5 rounded-2xl shadow-2xl flex items-center justify-between hover:bg-black transition-colors cursor-pointer border border-gray-800"
          >
                ...
                >
                  ...
                  <div className="flex flex-col items-end gap-3">
                    ...
                    <div className="flex items-center gap-3 bg-gray-100 px-2 py-1 rounded-xl font-bold">
                      <button
                        onClick={() => removeFromCart(item.name)}
                        className="w-7 h-7 bg-white rounded-lg shadow-sm flex items-center justify-center text-gray-600 hover:text-orange-500 active:scale-90 transition"
                      >
